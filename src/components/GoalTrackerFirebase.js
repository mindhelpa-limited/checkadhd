...
// --- PERSISTENCE HELPER FUNCTIONS (Updated for safety) ---
...
export default function GoalTrackerFirebase() {
  ...
  const [currentDateTime, setCurrentDateTime] = useState(''); // State for time/date header
  
  const { width, height } = useWindowSize();

  // IMPORTANT: auth.currentUser will be null initially on client-side mount
  ...
  }, [userId, startOfFilteredDay]); 


  // --- FIREBASE WRITE/UPDATE FUNCTIONS ---
  ...
  const addNewGoal = async (type, title, index) => {
    ...
    // Clear saved draft for this index after it's successfully saved to Firebase
    if (type === 'daily') {
  ...
  const renderGoalAccordions = (type) => {
    ...
    const canEditOrToggle = !isDaily || isToday; 
  ...
  const renderDailyGoalForm = () => {
    ...
    return renderGoalAccordions('daily');
  };
  
  const renderWeeklyGoalForm = () => renderGoalAccordions('weekly');
