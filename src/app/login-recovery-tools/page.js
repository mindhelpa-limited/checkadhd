...
export default function LoginPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        // ✅ Redirect to /dashboard/goal-tracker upon login
        router.push("/dashboard/goal-tracker"); 
      } else {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ✅ Redirect to /dashboard/goal-tracker after successful sign-in
      router.push("/dashboard/goal-tracker"); 
  ...
  return (
    <div className="min-h-screen w-full grid grid-cols-1 md:grid-cols-2 bg-[#0a122a] text-white">
      ...
      <div className="hidden md:flex flex-col justify-center items-center p-10 text-center">
        ...
        {/* 📝 You can update this text if this login is for Recovery Tools */}
        <h1 className="text-3xl font-bold">
          Get Best Recovery Tools
        </h1>
        <p className="mt-4 text-gray-300 max-w-sm">
          Your personalized tools for focus, progress, and lasting change starts here.
        </p>
      </div>
