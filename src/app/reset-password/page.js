...
const LockIcon = () => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>);
...
export default function ResetPasswordPage() {
  ...
  const [step, setStep] = useState("enter-email"); // 'enter-email' or 'reset-password'
  const [actionCode, setActionCode] = useState("");

  // This effect runs when the page loads to check for a reset code in the URL.
  useEffect(() => {
    const oobCode = searchParams.get("oobCode");
    if (oobCode) {
      setLoading(true);
      // Verify the code is valid before showing the password reset form.
      verifyPasswordResetCode(auth, oobCode)
        .then((verifiedEmail) => {
          setEmail(verifiedEmail); // Pre-fill the email from the code
          setActionCode(oobCode);
          setStep("reset-password"); // Change the view to the reset form
          setLoading(false);
        })
        .catch((err) => {
  ...
  // Step 1: Send the initial reset email
  const handleSendResetLink = async (e) => {
    ...
    try {
      ...
      setMessage("Password reset email sent. Please check your inbox and click the link to continue.");
    } catch (err) {
