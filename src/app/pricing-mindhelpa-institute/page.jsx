...
import { BookOpen, Calendar, DollarSign, Users, Zap, Briefcase, ChevronDown, ChevronUp, Star } from 'lucide-react';
...
const curriculumData = [
  ...
  },
  {
    ...
    modules: [
      ...
      // FIXED: Ensuring $\text{DSM-5}$ is properly part of the string
    ...
    <div className="border-b border-gray-700/50 last:border-b-0">
      ...
      >
        <div className="flex items-center space-x-4">
          <span className="text-xl font-bold text-cyan-400">{item.id}.</span>
          {/* Use item.title for the title as it's now a properly concatenated string */}
          <span className="text-white font-medium flex-1">{item.title}</span>
        </div>
      ...
      {isOpen && (
        <div className="px-6 pb-5 pt-0 text-gray-300 bg-blue-900/20 border-t border-gray-700/50 text-sm leading-relaxed">
          {item.content}
        </div>
      )}
    </div>
...
export default function MindhelpaInstitutePricing() {
  ...
  return (
    <div className="bg-[#040813] text-white min-h-screen pt-16 font-sans">
      ...
      <div className="max-w-6xl mx-auto text-center py-16 px-4">
        ...
        <h2 className="text-xl sm:text-3xl font-extralight text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-cyan-200 mt-2 italic">
      ...
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-3 gap-12 pb-20">
        
        {/* Price Card (Fixed mobile overlap by removing 'sticky') */}
        <div className="lg:col-span-1 p-8 rounded-3xl shadow-3xl bg-gradient-to-br from-blue-900 via-blue-900/80 to-blue-950 border border-blue-600/50 h-fit lg:sticky lg:top-8 transform hover:scale-[1.02] transition duration-300">
            <p className="text-sm uppercase tracking-widest font-bold text-blue-200 mb-2">Full Certification Program</p>
            ...
            <ul className="space-y-4 text-sm font-medium text-blue-100/90">
                <li className="flex items-center space-x-3">
                    <Calendar className="w-5 h-5 text-cyan-300 flex-shrink-0" /> 
                    <span>{'4 Weeks of Intensive Training ($\\text{20 Modules}$)'}</span>
                </li>
                <li className="flex items-center space-x-3">
                    <Zap className="w-5 h-5 text-cyan-300 flex-shrink-0" /> 
                    <span>{'$\\text{100\\%}$ Self-Paced Learning Model for Flexibility'}</span>
                </li>
                <li className="flex items-center space-x-3">
                    <Users className="w-5 h-5 text-cyan-300 flex-shrink-0" /> 
                    {/* Fixed: ensuring $\text{90-min}$ is correct */}
                    <span>{'4 x Weekly $\\text{90-min}$ Live Q\&A \& Mentorship Sessions'}</span>
                </li>
                <li className="flex items-center space-x-3">
                    <Briefcase className="w-5 h-5 text-cyan-300 flex-shrink-0" /> 
            ...
            <button
                ...
                className="mt-10 w-full glow-on-hover bg-gradient-to-r from-cyan-400 to-teal-400 hover:from-cyan-500 hover:to-teal-500 text-gray-900 text-lg font-extrabold py-4 rounded-xl transition-all duration-300 shadow-xl shadow-cyan-500/40 uppercase tracking-wider"
            >
                Secure Your Spot $\rightarrow$
            </button>
            <p className="mt-4 text-center text-xs text-blue-300/70">
                Investment in a professional, ethical, and profitable practice.
            </p>
        </div>

        {/* Curriculum Details (Deeper Background and Structure) */}
        <div className="lg:col-span-2 bg-blue-900/40 p-8 rounded-3xl border border-blue-700/50 shadow-inner shadow-blue-950/50">
          <h3 className="text-3xl font-bold mb-8 border-b border-blue-700/50 pb-4 text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-cyan-200">
            {'The $\\text{20-Module}$ Curriculum Breakdown'}
          </h3>
          
          <div className="space-y-6">
            {curriculumData.map((weekData, weekIndex) => (
              <div key={weekIndex} className="bg-blue-900/60 rounded-xl overflow-hidden shadow-xl border border-blue-700/50">
          ...
          <div className="mt-12 p-6 bg-cyan-900/30 rounded-xl border border-cyan-500/50 shadow-md shadow-cyan-900/50">
            <h4 className="text-2xl font-bold text-cyan-300 mb-4 flex items-center space-x-3">
                <Briefcase className="w-7 h-7" />
                <span>Certified Business Launchpad</span>
            </h4>
            <p className="text-gray-200 text-lg mb-4">
                We don't just train practitioners; we train successful entrepreneurs. Your certification includes:
            </p>
