...
// import Image from 'next/image'; // ❌ REMOVED: Using standard <img> for better print compatibility
...
export default function Report({ userInfo, answers }) {
    const { scorePercentage, level, riskColor, riskLevelText, interpretationText, scaleIndicatorPos } =
        getScoreDetails(answers);

    const formattedDob = new Intl.DateTimeFormat('en-GB', {
        day: '2-digit', month: '2-digit', year: 'numeric'
    }).format(new Date(userInfo.dob));

    const assessmentDate = new Intl.DateTimeFormat('en-US', {
        month: '2-digit', day: '2-digit', year: 'numeric'
    }).format(new Date());

    const assessmentTime = new Intl.DateTimeFormat('en-US', {
        ...
        <div id="report-content" className={styles.onScreenReportWrapper}>
            <div className={styles.pdfHeader}>
                <div className={styles.logoContainerPdf}>
                    {/* ✅ CRITICAL FIX: Using standard <img> tag for maximum print compatibility */}
                    <img
                        src="/logo.png" 
                        alt="ADHD Check Logo"
                        width={160}
                        height={40}
                        className={styles.logoImagePdf}
                    />
            ...
            <div className={styles.patientInfo}>
                ...
                <div className={styles.infoGrid}>
                    ...
                    <div className={styles.infoItem}>
                        ...
                        <span className={styles.infoValue}>{userInfo.sex}</span>
                    </div>
                    <div className={styles.infoItem}>
                        <span className={styles.infoLabel}>Assessment Date</span>
                        <span className={styles.infoValue}>{assessmentDate}</span>
                    </div>
                    <div className={styles.infoItem}>
                        <span className={styles.infoLabel}>Assessment Time</span>
                        <span className={styles.infoValue}>{assessmentTime}</span>
                    </div>
                </div>
            </div>
            ...
            <div className={styles.resultsContainer}>
                ...
                <div className={styles.likelihoodDisplay}>
                    <p className={styles.likelihoodPercentage} style={{ color: riskColor }}>
                        {scorePercentage}%
                    </p>
                    <p className={styles.likelihoodLabel}>{level}</p>
                    <div className={styles.riskLevel} style={{ backgroundColor: riskColor }}>
                        {riskLevelText}
                    </div>
                </div>
                ...
                <div className={styles.visualScale}>
                    <div className={styles.scaleContainer}>
                         {/* ✅ RETAINED: This is required for the fixed CSS scale to draw the colors */}
                        <div className={styles.scaleMiddle} /> 
                        <div
                            className={styles.scaleIndicator}
                            style={{ left: `${scaleIndicatorPos}%`, border: `3px solid ${riskColor}` }}
                        />
                    </div>
                    <div className={styles.scaleLabels}>
                        <span>Low (0-39%)</span>
                        <span>Moderate (40-74%)</span>
            ...
            <div className={`${styles.interpretationSection} ${styles.section}`}>
                ...
                <ul className={styles.list}>
                    {interpretationText.map((item, index) => (
            ...
            <div className={`${styles.recommendationsSection} ${styles.section}`}>
                ...
                <ul className={styles.list}>
                    <li className={styles.listItem}>
                        <strong>Consult a Professional:</strong> Discuss these results with a doctor, psychiatrist, or psychologist for a comprehensive clinical evaluation.
                    </li>
                    <li className={styles.listItem}>
                        <strong>Behavioral Strategies:</strong> Explore time management techniques, organizational systems, and minimizing distractions.
                    </li>
                    <li className={styles.listItem}>
                        <strong>Lifestyle Adjustments:</strong> Regular physical exercise, a balanced diet, and adequate sleep can significantly impact ADHD symptoms.
                    </li>
                </ul>
            </div>
            ...
            <div className={`${styles.importantNotes} ${styles.section}`}>
                ...
                <ul className={styles.list}>
                    <li className={styles.listItem}>
                        <strong>Screening Tool Only:</strong> This is an informational self-assessment tool. It does not provide a clinical diagnosis.
                    </li>
                    <li className={styles.listItem}>
                        <strong>Professional Evaluation Required:</strong> For an accurate diagnosis, please consult with a qualified healthcare provider.
                    </li>
                </ul>
            </div>
            ...
            <div className={styles.pdfFooter}>
                ...
                <p className={styles.confidential}>
                    This report is confidential and intended for the assessed individual's review.
                </p>
                <p style={{ marginTop: '10px', fontSize: '10px', color: '#9ca3af' }}>
                    ©{new Date().getFullYear()} Mindhelpa Ltd. All rights reserved.
                </p>
            </div>
        </div>
    );
