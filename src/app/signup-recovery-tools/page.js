...
const PaymentModal = () => (
  <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
    <div className="bg-[#101b3d] rounded-2xl shadow-lg max-w-md w-full p-6 text-center border border-white/10">
      ...
      <button
        onClick={() => (window.location.href = "/pricing-mental-health-recovery-tools")}
...
const LoginModal = ({ email, onClose, router }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      router.push("/dashboard/goal-tracker"); // ✅ updated redirect
    } catch (err) {
...
export default function SignUpRecoveryToolsPageWrapper() {
  return (
    <Suspense fallback={<FullScreenLoader message="Loading..." />}>
      <SignUpRecoveryToolsPage />
    </Suspense>
  );
}

function SignUpRecoveryToolsPage() {
  ...
  const productTag = "recovery"; // ✅ updated
  ...
  const redirectToDashboard = () => router.push("/dashboard/goal-tracker"); // ✅ updated
      ...
      {!showPaymentModal && !accountExists && (
        <div className="max-w-md w-full bg-[#101b3d] rounded-3xl shadow-2xl p-8 border border-white/10">
          ...
          <div className="text-center mt-6">
            <button
              onClick={() => router.push("/login-recovery-tools")}
